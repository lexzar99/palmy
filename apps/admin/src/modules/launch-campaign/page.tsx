  ...
  return (
    <div className="page-stack">
      ...
      <Surface className="px-5 py-5">
        ...
        <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[360px] text-left text-sm responsive-table"><thead><tr className="border-b border-[var(--border-subtle)] text-xs text-[var(--text-muted)]"><th scope="col" className="px-3 py-3 font-bold">Datum</th><th scope="col" className="px-3 py-3 font-bold">Nya leads</th></tr></thead><tbody>{data.daily.slice(-14).reverse().map((row) => <tr key={row.date} className="border-b border-[var(--border-subtle)] last:border-0"><td data-label="Datum" className="px-3 py-3 font-semibold">{row.date}</td><td data-label="Nya leads" className="px-3 py-3 font-black text-[var(--accent-ink)]">{formatNumber(row.leads)}</td></tr>)}</tbody></table></div>
      </Surface>
      ...
      <Surface className="px-5 py-5">
        ...
        {data.leads.length === 0 ? <p className="mt-6 rounded-2xl bg-[var(--surface-muted)] px-4 py-8 text-center text-sm font-semibold text-[var(--text-secondary)]">Inga registrerade leads ännu.</p> : <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm responsive-table"><thead><tr className="border-b border-[var(--border-subtle)] text-xs text-[var(--text-muted)]"><th scope="col" className="px-3 py-3 font-bold">Namn</th><th scope="col" className="px-3 py-3 font-bold">E-post</th><th scope="col" className="px-3 py-3 font-bold">Reserverad kupong</th><th scope="col" className="px-3 py-3 font-bold">Manuell status</th><th scope="col" className="px-3 py-3 font-bold">Registrerad</th><th scope="col" className="px-3 py-3 font-bold text-right">Åtgärd</th></tr></thead><tbody>{data.leads.map((lead) => <tr key={lead.id} className="border-b border-[var(--border-subtle)] last:border-0"><td data-label="Namn" className="px-3 py-3 font-bold">{lead.name}</td><td data-label="E-post" className="px-3 py-3 text-[var(--text-secondary)]">{lead.email}</td><td data-label="Reserverad kupong" className="px-3 py-3 font-mono text-xs">{lead.couponCode}</td><td data-label="Manuell status" className="px-3 py-3"><Badge tone={lead.couponSentAt ? "success" : "warning"}>{lead.couponSentAt ? "Markerad skickad" : "Väntar på kontakt"}</Badge></td><td data-label="Registrerad" className="px-3 py-3 text-[var(--text-secondary)]">{new Date(lead.createdAt).toLocaleString("sv-SE")}</td><td data-label="Åtgärd" className="px-3 py-3 text-right"><Button variant="secondary" disabled={markCoupon.isPending} onClick={() => markCoupon.mutate({ leadId: lead.id, sent: !lead.couponSentAt })}>{lead.couponSentAt ? "Ångra markering" : "Markera manuellt skickad"}</Button></td></tr>)}</tbody></table></div>}
