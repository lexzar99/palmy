      ...
      <div className="max-w-[1400px] mx-auto space-y-12 pb-32">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
           <div className="space-y-4">
              <Link href="/menu" className="flex items-center gap-2 text-[var(--text-primary)]/20 hover:text-[var(--text-primary)] transition-all text-xs font-black uppercase tracking-widest pl-1">
              ...
              <div>
                 ...
                 <p className="text-[var(--text-primary)]/40 text-[11px] font-black uppercase tracking-widest mt-2 ml-1">Hantera sortiment och priser</p>
              </div>
           </div>

           <div className="flex flex-wrap items-center gap-4">
              <div className="flex gap-2 p-1.5 bg-[var(--border-subtle)] border border-[var(--border-strong)] rounded-2xl">
                 {(["PRODUCTS", "CATEGORIES", "EXTRAS", "DEALS"] as Tab[]).map((tab) => (
                    <button key={tab} onClick={() => { setActiveTab(tab); setSearchTerm(""); }} className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? "bg-gold-500 text-dark-500 shadow-xl shadow-gold-500/10" : "text-[var(--text-primary)]/30 hover:text-[var(--text-primary)] hover:bg-[var(--border-subtle)]"}`}>
              ...
              <div className="flex gap-2 p-1.5 bg-[var(--border-subtle)] border border-[var(--border-strong)] rounded-2xl group">
                 <button onClick={() => router.push('/menu/import')} className="p-2.5 hover:bg-[var(--border-subtle)] rounded-xl transition-all text-[var(--text-primary)]/40 hover:text-gold-500" title="Bulk Import"><Upload size={16} /></button>
                 <button onClick={() => {/* handle sync */}} className="p-2.5 hover:bg-[var(--border-subtle)] rounded-xl transition-all text-[var(--text-primary)]/40 hover:text-gold-500" title="Synka externa tjänster"><Layers size={16} /></button>
              </div>

              <div className="relative group">
                 <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-primary)]/20 group-focus-within:text-gold-500 transition-colors" size={16} />
                 <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Sök..." className="bg-[var(--border-subtle)] border border-[var(--border-strong)] rounded-2xl py-3 pl-10 pr-6 outline-none focus:border-gold-500/40 transition-all text-xs w-40 lg:w-48" />
              </div>
              <button 
                onClick={() => {
        ...
        {loading ? (
           <div className="py-40 flex flex-col items-center justify-center gap-6">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[var(--text-primary)]/20">Hämtar data...</p>
           </div>
        ) : (
                 ...
                 <div key={p.id} className="glass border border-[var(--border-subtle)] rounded-[2.5rem] p-6 group hover:border-gold-500/30 transition-all flex flex-col h-full">
                    <div className="flex gap-4 mb-6">
                       ...
                       <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] font-bold text-[var(--text-primary)]/20 mt-1 line-clamp-2 leading-relaxed">{p.description || "Ingen beskrivning"}</p>
                       </div>
                    </div>
                    <div className="mt-auto pt-6 border-t border-[var(--border-subtle)] flex items-center justify-between">
                       ...
                       <div className="flex items-center gap-2">
                          <button onClick={() => openProductModal(p)} className="p-2.5 bg-[var(--border-subtle)] hover:bg-white/10 rounded-xl transition-all text-[var(--text-primary)]/40 hover:text-[var(--text-primary)]"><Edit2 size={14} /></button>
              ...
              <form onSubmit={handleSaveProduct} className="p-10 space-y-8">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-primary)]/20 ml-2">Namn</label>
                       <input required value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})} className="w-full bg-[var(--border-subtle)] border border-[var(--border-strong)] rounded-2xl px-6 py-4 text-sm font-black focus:border-gold-500/40 outline-none" />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-primary)]/20 ml-2">Pris</label>
                       <input required type="number" value={productForm.price || ""} onChange={e => setProductForm({...productForm, price: parseInt(e.target.value) || 0})} className="w-full bg-[var(--border-subtle)] border border-[var(--border-strong)] rounded-2xl px-6 py-4 text-sm font-black focus:border-gold-500/40 outline-none" />
                    </div>
                 ...
                 <div className="pt-6 flex gap-4">
                    <button type="button" onClick={() => setIsProductModalOpen(false)} className="flex-1 py-4 bg-[var(--border-subtle)] rounded-2xl text-[10px] font-black uppercase tracking-widest">Avbryt</button>
