    ...
    <div className="space-y-10 pb-24">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        <div className="space-y-6">
          <div>
            ...
            <p className="text-white/40 font-medium tracking-wide">Hantera produkter, kategorier och tillbehör på samma ställe.</p>
          </div>
            ...
            {(["PRODUCTS", "CATEGORIES", "EXTRAS", "DEALS"] as Tab[]).map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                  activeTab === tab ? "bg-gold-500 text-dark-500 shadow-lg shadow-gold-500/20" : "text-white/40 hover:text-white"
                }`}
              >
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-white/10 transition-all disabled:opacity-50"
          >
          ...
          <div className="relative group">
            ...
            <input 
              ...
              className="bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-6 outline-none focus:ring-2 focus:ring-gold-500/30 transition-all text-sm w-64"
            />
          </div>
          <button 
            onClick={() => {
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Kategorier</div>
              <div className="text-3xl font-black text-gold-500">{categories.length}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Produkter</div>
              <div className="text-3xl font-black text-gold-500">{products.length}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Tillbehörsgrupper</div>
              <div className="text-3xl font-black text-gold-500">{extraGroups.length}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Aktiva deals</div>
          ...
          {activeTab === "PRODUCTS" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} key="prod" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredProducts.map(p => (
                <div key={p.id} className="bg-white/5 border border-white/5 rounded-3xl p-6 group hover:border-gold-500/30 transition-all relative overflow-hidden flex flex-col h-full">
                  <div className="flex gap-4 mb-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-white/40 mt-1 line-clamp-2">{p.description || "Ingen beskrivning"}</p>
                      <div className="mt-2 text-[9px] font-black uppercase tracking-widest text-white/20 bg-white/5 w-fit px-2 py-1 rounded">{p.category?.name}</div>
                    </div>
                  </div>
                  <div className="mt-auto pt-6 border-t border-dashed border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-white/20 text-[9px] font-bold uppercase">{p.extraGroups?.length || 0} Tillbehör</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <button onClick={() => openProductModal(p)} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all"><Edit2 size={16} /></button>
          ...
          {activeTab === "CATEGORIES" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} key="cat" className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredCategories.map(cat => (
                <div key={cat.id} className="bg-white/5 border border-white/5 rounded-[2rem] overflow-hidden group hover:border-gold-500/30 transition-all flex flex-col h-full">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <p className="text-xs text-white/40 line-clamp-2 mb-6 flex-1">{cat.description || "Ingen beskrivning..."}</p>
                    <div className="flex items-center justify-between pt-6 border-t border-white/5">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20">{cat._count?.products || 0} Art</span>
                      <div className="flex items-center gap-2">
                         <button onClick={() => openCategoryModal(cat)} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all"><Edit2 size={16} /></button>
          ...
          {activeTab === "EXTRAS" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} key="extra" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredExtras.map(group => (
                <div key={group.id} className="bg-white/5 border border-white/5 rounded-3xl p-8 group hover:border-gold-500/30 transition-all flex flex-col h-full">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex items-center gap-2">
                       <button onClick={() => openExtraModal(group)} className="p-2 bg-white/5 hover:bg-white/10 rounded-xl transition-all"><Edit2 size={16} /></button>
                  ...
                  <div className="space-y-2 flex-1">
                    {group.extras.map((e: any) => (
                      <div key={e.id} className="flex justify-between items-center text-xs p-3 bg-white/5 rounded-xl border border-white/5">
          ...
          {activeTab === "DEALS" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} key="deals" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredDeals.map((deal) => (
                <div key={deal.id} className="bg-white/5 border border-white/5 rounded-[2rem] p-8 hover:border-gold-500/30 transition-all flex flex-col h-full">
                  <div className="flex items-start justify-between gap-4 mb-6">
                    <div>
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        {deal.badgeText && (
                          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 bg-white/5 px-3 py-1 rounded-full">
                            {deal.badgeText}
                          </span>
                        )}
                      </div>
                      <h3 className="text-2xl font-black tracking-tight">{deal.title}</h3>
                      <p className="text-white/40 text-sm mt-2 leading-relaxed">{deal.description || "Ingen beskrivning ännu."}</p>
                    </div>
                    <button
                      onClick={() => toggleDealActive(deal)}
                      className={`p-3 rounded-2xl border transition-all ${deal.isActive ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-300" : "bg-white/5 border-white/10 text-white/40"}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className="rounded-2xl bg-white/5 border border-white/5 p-4">
                      <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Belöning</div>
                      <div className="text-lg font-black text-gold-500">{formatDealValue(deal)}</div>
                    </div>
                    <div className="rounded-2xl bg-white/5 border border-white/5 p-4">
                      <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Gäller till</div>
                      <div className="text-sm font-bold text-white">{deal.validUntil ? new Date(deal.validUntil).toLocaleDateString("sv-SE") : "Tills vidare"}</div>
                    </div>
                  </div>

                  <div className="space-y-3 text-sm text-white/55 flex-1">
                    {deal.minOrder > 0 && (
                    ...
                    {deal.comboProductNames?.length > 0 && (
                      <div className="rounded-2xl bg-white/5 border border-white/5 px-4 py-3">
                        <div className="text-[10px] uppercase tracking-[0.3em] text-white/20 mb-2">Combo</div>
                        <div className="flex flex-wrap gap-2">
                          {deal.comboProductNames.map((name: string) => (
                            <span key={name} className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60 bg-dark-500 px-3 py-1 rounded-full">
                              {name}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="rounded-2xl bg-white/5 border border-white/5 px-4 py-3 flex items-center justify-between">
                      <span>Visas på sidan</span>
                      <span className={deal.showOnSite ? "text-emerald-300 font-bold" : "text-white/30 font-bold"}>
                        {deal.showOnSite ? "Ja" : "Nej"}
                      </span>
                    </div>
                    <div className="rounded-2xl bg-white/5 border border-white/5 px-4 py-3 flex items-center justify-between">
                      <span>Per kund</span>
                      <span className="font-bold text-white">{deal.maxUsesPerCustomer || "Obegränsat"}</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => openDealModal(deal)} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all">
                        <Edit2 size={16} />
                      </button>
      ...
      {isProductModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-dark-500/90 backdrop-blur-md">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-dark-400 border border-white/10 rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl shadow-black">
             <div className="p-10 border-b border-white/5 flex justify-between items-center bg-dark-400 z-10 sticky top-0">
               <div>
                  <h2 className="text-3xl font-black uppercase tracking-tight">{editingId ? 'Redigera Artikel' : 'Ny Artikel'}</h2>
                  <p className="text-white/40 text-sm mt-1">Ställ in priser, tillbehör och kategorier.</p>
               </div>
               <button onClick={() => setIsProductModalOpen(false)} className="p-4 bg-white/5 rounded-full hover:bg-white/10 transition-colors"><X size={24} /></button>
             </div>
             ...
             <div className="p-10 overflow-y-auto no-scrollbar flex-1">
                <form onSubmit={handleSaveProduct} className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-8">
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Artikelnamn</label>
                      <input required value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50" placeholder="t.ex. Margherita" />
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Pris (KR)</label>
                        <input required type="number" value={productForm.price || ""} onChange={e => setProductForm({...productForm, price: parseInt(e.target.value) || 0})} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50" placeholder="100" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Kategori</label>
                        <select value={productForm.categoryId} onChange={e => setProductForm({...productForm, categoryId: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50 appearance-none">
                          {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Beskrivning</label>
                      <textarea value={productForm.description} onChange={e => setProductForm({...productForm, description: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50 h-32 resize-none" placeholder="Ingredienser..." />
                    </div>
                  </div>

                  <div className="space-y-8">
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Bild</label>
                      <div className="flex gap-4">
                        <div className="w-24 h-24 bg-white/5 rounded-2xl flex items-center justify-center border border-dashed border-white/10 overflow-hidden">
                        ...
                        <div className="flex-1 flex flex-col gap-2">
                           <input type="text" value={productForm.imageUrl} onChange={e => setProductForm({...productForm, imageUrl: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-xl py-3 px-4 text-xs" placeholder="URL till bild..." />
                           <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 justify-center w-full py-3 bg-white/5 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest border border-white/5">
                    ...
                    <div className="pt-4 border-t border-white/5">
                       <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-4 ml-1">Koppla Tillbehör</label>
                          ...
                          {extraGroups.map(g => {
                            ...
                            return (
                              <button
                                ...
                                className={`p-4 rounded-2xl border text-sm font-bold transition-all text-left flex justify-between items-center ${isSelected ? "bg-gold-500/10 border-gold-500 text-gold-500" : "bg-white/5 border-white/5 text-white/40 hover:bg-white/10"}`}
                              >
                                {g.name}
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-dark-500/90 backdrop-blur-md">
           <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-dark-400 border border-white/10 rounded-[2.5rem] w-full max-w-lg shadow-2xl">
              <div className="p-10 border-b border-white/5 flex justify-between items-center">
                 <h2 className="text-2xl font-black uppercase tracking-tight">{editingId ? 'Redigera Kategori' : 'Ny Kategori'}</h2>
                 <button onClick={() => setIsCategoryModalOpen(false)} className="p-3 bg-white/5 rounded-full"><X size={20} /></button>
              </div>
              <form onSubmit={handleSaveCategory} className="p-10 space-y-6">
                 <div>
                   <label className="block text-[10px] font-black uppercase text-white/20 mb-2">Namn</label>
                   <input required value={categoryForm.name} onChange={e => setCategoryForm({...categoryForm, name: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" placeholder="t.ex. Kebabrullar" />
                 </div>
                 <div>
                   <label className="block text-[10px] font-black uppercase text-white/20 mb-2">Bild URL</label>
                   <input value={categoryForm.imageUrl} onChange={e => setCategoryForm({...categoryForm, imageUrl: e.target.value})} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" placeholder="https://..." />
                 </div>
      ...
      {isExtraModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-dark-500/90 backdrop-blur-md">
           <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-dark-400 border border-white/10 rounded-[2.5rem] w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
              <div className="p-10 border-b border-white/5 flex justify-between items-center">
                 <h2 className="text-2xl font-black uppercase tracking-tight">{editingId ? 'Redigera Grupp' : 'Ny Grupp'}</h2>
                 <button onClick={() => setIsExtraModalOpen(false)} className="p-3 bg-white/5 rounded-full"><X size={20} /></button>
              </div>
              <div className="p-10 overflow-y-auto flex-1 no-scrollbar space-y-8">
                <form id="extraForm" onSubmit={handleSaveExtra} className="space-y-8">
                   <div className="grid grid-cols-2 gap-6">
                      <input required value={extraForm.name} onChange={e => setExtraForm({...extraForm, name: e.target.value})} className="bg-white/5 border border-white/5 rounded-2xl p-4 outline-none" placeholder="Gruppnamn..." />
                      <select value={extraForm.type} onChange={e => setExtraForm({...extraForm, type: e.target.value})} className="bg-white/5 border border-white/5 rounded-2xl p-4 outline-none">
                   ...
                   <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <label className="text-[10px] font-black uppercase text-white/20 tracking-widest">Tillbehörsrader</label>
              ...
              <div className="p-8 bg-dark-400 border-t border-white/5">
      ...
      {isDealModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-dark-500/90 backdrop-blur-md">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-dark-400 border border-white/10 rounded-[2.5rem] w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl shadow-black">
            <div className="p-10 border-b border-white/5 flex justify-between items-center">
              <div>
                <h2 className="text-3xl font-black uppercase tracking-tight">{editingId ? "Redigera Deal" : "Ny Deal"}</h2>
                <p className="text-white/40 text-sm mt-1">Bygg riktiga deals som syns på sidan och kan appliceras automatiskt.</p>
              </div>
              <button onClick={() => setIsDealModalOpen(false)} className="p-4 bg-white/5 rounded-full hover:bg-white/10 transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-10 overflow-y-auto no-scrollbar flex-1">
              <form onSubmit={handleSaveDeal} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Titel</label>
                    <input required value={dealForm.title} onChange={(e) => setDealForm({ ...dealForm, title: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50" placeholder="T.ex. 10% på hela ordern" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Badge</label>
                    <input value={dealForm.badgeText} onChange={(e) => setDealForm({ ...dealForm, badgeText: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50" placeholder="T.ex. Helgens deal" />
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Beskrivning</label>
                  <textarea value={dealForm.description} onChange={(e) => setDealForm({ ...dealForm, description: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none focus:ring-2 focus:ring-gold-500/50 h-28 resize-none" placeholder="Kort text som kunden ser i menyn och carten." />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Dealtyp</label>
                    <select value={dealForm.dealType} onChange={(e) => setDealForm({ ...dealForm, dealType: e.target.value, comboProductIds: e.target.value === "COMBO" ? dealForm.comboProductIds : [] })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none">
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Belöning</label>
                    <select value={dealForm.discountType} onChange={(e) => setDealForm({ ...dealForm, discountType: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none">
                      <option value="PERCENTAGE">%</option>
                      <option value="FIXED">Fast kr</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">{dealForm.discountType === "FIXED" ? "Rabatt (KR)" : "Rabatt (%)"}</label>
                    <input type="number" min={1} value={dealForm.discountValue || ""} onChange={(e) => setDealForm({ ...dealForm, discountValue: parseInt(e.target.value) || 0 })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Minimiorder</label>
                    <input type="number" min={0} value={dealForm.minOrder || ""} onChange={(e) => setDealForm({ ...dealForm, minOrder: parseInt(e.target.value) || 0 })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" />
                  </div>
                </div>

                {dealForm.dealType === "COMBO" && (
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Combo-produkter</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Giltig till</label>
                    <input type="datetime-local" value={dealForm.validUntil} onChange={(e) => setDealForm({ ...dealForm, validUntil: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Sortering</label>
                    <input type="number" min={0} value={dealForm.sortOrder || 0} onChange={(e) => setDealForm({ ...dealForm, sortOrder: parseInt(e.target.value) || 0 })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" />
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Max användningar totalt</label>
                    <input type="number" min={1} value={dealForm.maxUsages} onChange={(e) => setDealForm({ ...dealForm, maxUsages: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" placeholder="Tomt = obegränsat" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-white/20 mb-3 ml-1">Per kund</label>
                    <input type="number" min={1} value={dealForm.maxUsesPerCustomer} onChange={(e) => setDealForm({ ...dealForm, maxUsesPerCustomer: e.target.value })} className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-6 outline-none" placeholder="Tomt = obegränsat" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    ...
                    <button
                      ...
                      className={`p-4 rounded-2xl border text-left transition-all ${(dealForm as any)[option.key] ? "bg-gold-500/10 border-gold-500 text-gold-500" : "bg-white/5 border-white/5 text-white/50"}`}
                    >
