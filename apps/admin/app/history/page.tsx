      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-3">
           ...
           <div className="text-right">
              ...
              <div className="text-xl font-black text-[var(--text-primary)] tabular-nums">{data.total} <span className="text-[10px] text-[var(--text-primary)]/20">KR</span></div>
           </div>
        </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={o.id} onClick={() => setExpandedId(expandedId === o.id ? null : o.id)} className="glass border border-[var(--border-subtle)] rounded-[2rem] p-6 cursor-pointer hover:border-gold-500/10 hover:bg-[var(--bg-secondary)] transition-all group shadow-2xl relative overflow-hidden">
               ...
               <div className="flex items-center justify-between gap-6 relative z-10">
                <div className="flex items-center gap-5">
                  <div className="w-10 h-10 bg-[var(--border-subtle)] group-hover:bg-gold-500/10 rounded-xl flex items-center justify-center font-black text-[var(--text-primary)]/20 group-hover:text-gold-500 transition-colors italic text-xs shadow-inner">#{o.orderNumber}</div>
                  <div>
                        ...
                        <div className="space-y-2">
                           {o.items?.map((it, idx) => (
                             <div key={idx} className="flex justify-between items-center bg-[var(--border-subtle)] p-4 rounded-xl border border-[var(--border-subtle)]">
                                <span className="font-black text-xs text-[var(--text-primary)]/80 uppercase"><span className="text-gold-500 mr-2">{it.quantity}x</span> {it.productName}</span>
                                <span className="text-[11px] font-bold text-[var(--text-primary)]/20">{it.price * it.quantity} KR</span>
                             </div>
                           ))}
                        </div>
                      </div>
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-32 pt-4 lg:pt-10 px-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl lg:text-4xl font-black uppercase text-[var(--text-primary)] tracking-tighter leading-none italic">Order<span className="text-gold-500">Historik</span></h1>
        ...
        <div className="flex gap-4">
           ...
           <div className="glass border border-[var(--border-subtle)] p-5 rounded-[2rem] text-center min-w-[130px] shadow-2xl relative overflow-hidden group">
              ...
              <div className="text-xl font-black text-[var(--text-primary)]/80 tabular-nums">{groups.yesterday.total} KR</div>
           </div>
        </div>
      </div>
      ...
      <div className="space-y-24">
        {loading && orders.length === 0 ? (
          ...
          <>
            ...
            {renderGroup("Historik", groups.older, "text-[var(--text-primary)]/40", "bg-white/10")}
            
            {orders.length === 0 && !loading && (
              <div className="py-32 text-center text-[var(--text-primary)]/5 italic uppercase font-black tracking-[1em] text-xs">Ingen Historik</div>
            )}
          </>
        )}
      </div>
