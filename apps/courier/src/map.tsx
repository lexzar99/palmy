...
const pin = (emoji: string, bg: string) =>
  L.divIcon({
    className: "",
    html: `<div style="display:flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background:${bg};box-shadow:0 2px 6px rgba(0,0,0,.3)">
...
export function LiveMap({ me, pickup, dropoff, height = 220 }: { me?: LatLng | null; pickup?: LatLng; dropoff?: LatLng; height?: number }) {
  ...
  useEffect(() => {
    ...
    if (pickup) {
      L.marker([pickup.lat, pickup.lng], { icon: pin("🍽️", "#0C0B0C") }).addTo(map);
      pts.push([pickup.lat, pickup.lng]);
    }
    if (dropoff) {
      L.marker([dropoff.lat, dropoff.lng], { icon: pin("🏠", "#0C0B0C") }).addTo(map);
