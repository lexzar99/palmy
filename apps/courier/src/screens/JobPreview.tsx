...
import { ArrowLeft, Bike, Car, Clock, MapPin, Store } from "lucide-react";
import { api } from "../lib/api";
import { useGeo } from "../lib/geoctx";
import type { Job } from "../lib/types";
import { kr } from "../lib/format";
import { LiveMap } from "../map";
import { AddressRow, Card, GoldButton, Spinner } from "../ui";
    ...
    return (
      <div className="px-5 pt-3">
        <div className="mb-3 flex items-center gap-3">
          <button onClick={() => nav("/")} className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--color-line)] bg-white active:scale-95">
            <ArrowLeft size={18} />
          </button>
          <h1 className="text-lg font-black tracking-tight">Order</h1>
        </div>
        <Card className="p-8 text-center text-sm text-muted">Ordern är inte längre tillgänglig.</Card>
      </div>
    );
  }
  ...
  return (
    <div className="px-5 pt-3 pb-28">
      <div className="mb-3 flex items-center gap-3">
        ...
        <h1 className="truncate text-lg font-black tracking-tight">{job.restaurantName}</h1>
      </div>

      <LiveMap me={me} pickup={job.pickup} dropoff={job.dropoff} height={220} />
