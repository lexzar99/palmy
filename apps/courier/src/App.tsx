    ...
    <div className="flex min-h-screen flex-col justify-center px-6 pb-10">
      <div className="mx-auto w-full max-w-sm text-center">
        ...
        <p className="mt-2 text-sm text-muted">
          Vi behöver din position för att matcha dig med ordrar nära dig och visa dig vägen.
        </p>
        <div className="mt-5">
        ...
        {status === "denied" && (
          <p className="mt-3 text-sm font-medium text-red-500">
            Platsåtkomst nekad. Tillåt plats för Delivera i webbläsarens inställningar och försök igen.
          </p>
        )}
        {status === "unsupported" && (
          <p className="mt-3 text-sm font-medium text-red-500">Din webbläsare stöder inte platstjänster.</p>
        )}
      </div>
    </div>
  );
...
const TABS = [
  { to: "/", label: "Uppdrag", icon: "🚲" },
  { to: "/aktiv", label: "Pågående", icon: "📦" },
...
function BottomNav() {
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-20 mx-auto flex max-w-md items-stretch border-t border-[var(--color-line)] bg-white/95 backdrop-blur"
    ...
    >
      {TABS.map((t) => (
        <NavLink
          ...
          className={({ isActive }) =>
            `flex flex-1 flex-col items-center gap-1 py-2.5 text-[11px] font-bold ${isActive ? "text-gold-deep" : "text-muted"}`
          }
        >
    ...
    <GeoContext.Provider value={geo.coords}>
      <div className="mx-auto min-h-screen max-w-md bg-canvas" style={{ paddingTop: "env(safe-area-inset-top)" }}>
        <Routes>
          ...
          <Route path="/aktiv" element={<Active />} />
