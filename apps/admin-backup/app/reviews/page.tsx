...
export default function ReviewsPage() {
  ...
  const submitReply = async () => {
    const token = getStoredToken();
    if (!token || !replyingReview || !replyText.trim()) return;

    try {
      await axios.post(
        `${API_URL}/api/admin/reviews/${replyingReview.id}/reply`,
        { reply: replyText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setReviews((previous) => previous.map((review) => (review.id === replyingReview.id ? { ...review, reply: replyText } : review)));
  ...
  const removeReview = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/api/admin/reviews/${deleteTarget.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setReviews((previous) => previous.filter((review) => review.id !== deleteTarget.id));
    ...
    return (
      <div className="panel flex min-h-[320px] items-center justify-center rounded-[28px] px-6 py-12">
        <div className="flex items-center gap-3 text-[var(--text-secondary)]">
          ...
          <span className="text-sm font-semibold">Laddar recensioner...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="space-y-6 pb-16">
        <section className="panel rounded-[28px] px-6 py-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-3">
              <span className="control-chip">Enklare reviews</span>
              <div>
                <h2 className="text-3xl font-black tracking-[-0.06em] text-[var(--text-primary)] sm:text-4xl">Svar och radering öppnas i modal.</h2>
                <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--text-secondary)] sm:text-base">
                  Recensionerna ligger kvar som lista, men åtgärderna öppnas nu tydligt i modal i stället för inline-fält som flyttar runt hela sidan.
                </p>
              </div>
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {[
            ...
            { label: "Besvarade", value: metrics.replied, sub: "Har adminsvar" },
        ...
        <section className="panel rounded-[28px] px-6 py-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Sök kund, restaurang eller kommentar" className="control-input pl-10" />
            </div>

            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => setRatingFilter(null)} className={`rounded-2xl px-4 py-3 text-[11px] font-black uppercase tracking-[0.22em] ${ratingFilter === null ? "bg-gold-gradient text-[#091018]" : "border border-[var(--border-subtle)] bg-[var(--panel-muted)] text-[var(--text-secondary)]"}`}>
                Alla betyg
              </button>
              {[5, 4, 3, 2, 1].map((rating) => (
                <button key={rating} type="button" onClick={() => setRatingFilter(rating)} className={`rounded-2xl px-4 py-3 text-[11px] font-black uppercase tracking-[0.22em] ${ratingFilter === rating ? "bg-[rgba(245,191,91,0.18)] text-amber-100" : "border border-[var(--border-subtle)] bg-[var(--panel-muted)] text-[var(--text-secondary)]"}`}>
                  {rating}★
                </button>
              ))}
            </div>
          ...
          <div className="mt-5 grid gap-4">
            {filtered.length === 0 ? (
              ...
              filtered.map((review) => (
                <article key={review.id} className={`rounded-[24px] border px-5 py-5 ${review.flagged ? "border-amber-300/24 bg-amber-300/10" : "border-[var(--border-subtle)] bg-[var(--panel-muted)]"}`}>
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    <div className="space-y-3">
                      ...
                      {review.reply ? (
                        <div className="rounded-[20px] border border-[var(--border-subtle)] bg-[rgba(255,255,255,0.02)] px-4 py-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button type="button" onClick={() => { setReplyingReview(review); setReplyText(review.reply || ""); }} className="control-chip">
                        <MessageSquare size={13} /> {review.reply ? "Redigera svar" : "Svara"}
                      </button>
                      <button type="button" onClick={() => setDeleteTarget(review)} className="control-chip text-rose-200">
      ...
      <Modal open={!!replyingReview} onClose={() => { setReplyingReview(null); setReplyText(""); }} title={replyingReview ? `Svara: ${replyingReview.restaurantName}` : "Svara på recension"} maxWidth="max-w-2xl">
        <div className="grid gap-4">
          <div className="rounded-[20px] border border-[var(--border-subtle)] bg-[var(--panel-muted)] px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)]">Recension</p>
                <p className="mt-2 text-sm font-black text-[var(--text-primary)]">{replyingReview?.customerName}</p>
              </div>
              {replyingReview ? <Stars rating={replyingReview.rating} /> : null}
            </div>
            <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">{replyingReview?.comment || "Kunden lämnade ett betyg utan text."}</p>
          </div>

          <textarea value={replyText} onChange={(event) => setReplyText(event.target.value)} placeholder="Skriv ett tydligt svar till kunden" className="control-input min-h-[140px] resize-none" />

          <div className="flex flex-wrap items-center justify-end gap-2 pt-2">
            <button type="button" onClick={() => { setReplyingReview(null); setReplyText(""); }} className="control-chip">
              Avbryt
            </button>
            <button type="button" onClick={() => void submitReply()} className="inline-flex items-center gap-2 rounded-2xl bg-gold-gradient px-4 py-3 text-[11px] font-black uppercase tracking-[0.22em] text-[#091018]">
      ...
      <ConfirmModal
        ...
        message={`Radera recensionen från ${deleteTarget?.customerName || "kunden"}? Detta går inte att ångra.`}
        confirmLabel="Radera"
        danger
      />
    </>
