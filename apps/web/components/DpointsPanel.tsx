...
// Vpoints-panel i profilen: saldo, historik (bakom knapp) och sätt att tjäna.
...
export default function DpointsPanel() {
  ...
  async function load() {
    try {
      ...
      // tyst — panelen göms om Vpoints är av/otillgängligt
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="space-y-5">
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-zinc-600">+{me.signup.bonusPoints} Vpoints väntar på dig</p>
            </div>
      ...
      <div className="rounded-3xl bg-zinc-950 px-5 py-4 text-white">
        <div className="flex items-center gap-2 text-xs font-semibold text-zinc-400">
          <Coins className="h-4 w-4" style={{ color: "var(--color-gold-500, #F0531C)" }} /> Vpoints
        </div>
      ...
      {/* Tjäna Vpoints — styrs från admin (label + poäng + på/av). */}
      {earnRules.length > 0 && (
        <div>
          <h3 className="text-sm font-bold text-zinc-900">Tjäna Vpoints</h3>
