...
const DealSpotlight = ({ deals, subtotal, productIds, floating = false }: DealSpotlightProps) => {
  ...
  const body = (
    <div className={`relative overflow-hidden rounded-[2rem] border ${floating ? "bg-white/95" : "bg-[var(--bg-secondary)]"} p-6`} style={{ borderColor: 'var(--border-muted)' }}>
      {floating && (
        <button onClick={() => setDismissed(true)} className="absolute right-4 top-4 text-zinc-400/30 hover:text-zinc-600 transition-colors">
          <X size={16} />
        </button>
      )}
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="text-xl font-black tracking-tight" style={{ color: 'var(--text-primary)' }}>{spotlight.title}</h3>
          <p className="mt-2 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{spotlight.description || formatDealReward(spotlight)}</p>
        </div>
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gold-500 text-zinc-950 shadow-xl">
          <Gift size={20} />
        </div>
      </div>

      <div className="mb-4 flex items-center justify-between gap-4 text-sm">
        <div style={{ color: 'var(--text-secondary)' }}>{evaluation.progress.message}</div>
        <div className="font-bold text-gold-600">{formatDealReward(spotlight)}</div>
      </div>

      <div className="h-3 w-full overflow-hidden rounded-full border border-white/5 bg-zinc-100">
        <motion.div
          initial={{ width: 0 }}
      ...
      {spotlight.comboProductNames.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {spotlight.comboProductNames.map((name) => (
            <span key={name} className="rounded-full bg-zinc-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">
              {name}
            </span>
          ))}
        </div>
      ...
      {spotlight.validUntil && (
        <div className="mt-4 text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-400">
