    ...
    if (!coords && storeAddress) {
      (async () => {
        try {
          ...
          const res = await fetch(`https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(storeAddress)}&filter=countrycode:se&limit=1&apiKey=${GEOAPIFY_KEY}`);
