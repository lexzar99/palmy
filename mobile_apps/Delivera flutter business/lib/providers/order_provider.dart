...
class OrderProvider with ChangeNotifier {
  ...
  // Levererar restaurangen själv (true) eller plattformen åt den (false)?
  // Styr dynamiskt förvald tillagningstid + knapp-etikett. Default true =
  // bevarar nuvarande beteende (40 min / "Markera på väg") tills flaggan laddats.
  ...
  // ── Dynamisk leveransmodell ───────────────────────────────────────────────
  // Plattformen levererar (selfDelivery=false): förvalt 15 min, max 50.
  // Restaurangen levererar själv (selfDelivery=true): förvalt 40 min, max 90.
  // Avhämtning (PICKUP) påverkas inte — alltid 20 min.
  int defaultPrepMinutes(String orderType) {
    if (orderType == 'PICKUP') return 20;
    return _selfDelivery ? 40 : 15;
  }

  int maxPrepMinutes(String orderType) {
    if (orderType == 'PICKUP') return 90;
    return _selfDelivery ? 90 : 50;
  }

  /// Etikett + nästa status för "avancera order"-knappen. Plattformsleverans →
  /// restaurangen markerar "Klar för hämtning" (READY) så budet kan ta över.
  /// Self-leverans → "Markera på väg" (DELIVERING) som tidigare.
  String advanceLabel(String orderType) {
    if (orderType == 'PICKUP') return 'Klar för hämtning';
    return _selfDelivery ? 'Markera på väg' : 'Klar för hämtning';
  }

  String advanceNextStatus(String orderType) {
    if (orderType == 'PICKUP') return 'READY';
    return _selfDelivery ? 'DELIVERING' : 'READY';
  }
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (current != null) {
          ...
          if (current['selfDelivery'] is bool) {
            _selfDelivery = current['selfDelivery'];
          }
  ...
  Future<void> initSocket(String restaurantId) async {
    ...
    _socket!.on('settings:updated', (data) {
      ...
      if (data['restaurantId'] == _restaurantId ||
        ...
        // Admin bytte leveransmodell → byt knappar + förvald tid LIVE.
        if (data['selfDelivery'] is bool) {
          _selfDelivery = data['selfDelivery'];
          notifyListeners();
        }
