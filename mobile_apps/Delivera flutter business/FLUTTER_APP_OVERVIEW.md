...
**Delivera Business** — en order-taking app för restaurangpersonal (rebrand från MatGo/FoodGo/Levera).  
Version: `26.06.03+48`  
...
| 2026-06-02 | **hjälp/felsökning-sidan flat svartvit**: `printer_help_screen.dart` följde fortfarande gamla guld-temat (ember intro-kort, guld expand-kant, guld ikon-chips + steg-nummer, legacy `deepSea`/`mist`). Nu flat svartvit: header = eyebrow "HJÄLP" + titel + cirkel-knappar (bak/inställningar) som övriga skärmar; intro-kort + sektioner = `panelColor`/`faintColor` + `borderColor`; expanderad sektion får **ink-kant** (var guld); ikon-chips = faint + ink-ikon; steg-nummer = **solid ink-cirkel** med kontrastfärg; footer-noteringar neutrala. Ny `_CircleButton`. APK byggd. |
| 2026-06-02 | **ny-order-skärm matchar order-detalj-designen**: `order_take_screen.dart` byggd om så den ser likadan ut som order-detalj-sidan (som användaren tycker är fin): typ-raden = **tonad pill** med ikon (blå/guld/lila) + datum istället för platt blå text; ordernummer-raden visar nu **total** bredvid (`#1010 … 100 kr`); kund = namn (21 w900) + telefon + adress med en **ring-knapp** till höger (ink-tonad ruta) istället för inline-ikonrader; meddelande/instruktioner = **boxad callout** med vänster-kant + chat-ikon (+ allergener-block i rött) istället för "MEDDELANDE"-etikett + platt text. `_ContactLine` borttagen; nya `_TypePill`/`_CustomerBlock`/`_MessageCallout`/`_NoteRow` speglar order-detaljens komponenter. Bottom-bar (TID/KLAR + tid-strip + ACCEPTERA) + neka-knapp oförändrade. APK byggd. |
| 2026-06-02 | **order-detalj förenklad + större app-ikon-wordmark**: (1) **Order-detalj** (`order_detail_screen.dart`) städad till samma minimala kvitto-känsla som order-take: status-strip:en (dot-prickar + linjer + glow-skugga + per-steg-etiketter) ersatt med en **kompakt enradig segmenterad progress-bar** (STATUS · bar · nuvarande steg-ord), `_ProgressDot` borttagen; artiklar konverterade från accent-rutor (36px guld/ink-box + divider mellan rader) → **rena kvitto-rader** (`2× namn … pris`, extras indenterade "+ namn", item-notis kursiv) precis som order-take; sektions-dividers strammare (18→14px); "ARTIKLAR" → "BESTÄLLNING". (2) Order-take `_rule` 16→14px för matchande kompakthet. (3) **App-ikon**: adaptiv-foreground-wordmarket förstorat 70%→92% (efter 16%-inset ~62%) så texten "Levera. BUSINESS" syns tydligt — föregående var för utzoomad. APK byggd. |
| 2026-06-02 | **paus-bugg + viloläge/stängt-redesign + ikon/logga-fix**: (1) **Paus-bugg fixad**: efter "Pausa 30 min" → tryck × i viloläget → tillbaka på dashboard gick det inte att öppna igen (statusen fastnade på "Pausad"). `OrderProvider.setStatus(true)` rensar nu alltid en aktiv paus (`_pausedUntil`/timer + synk till backend); dashboardens "Öppet"-val anropar `cancelPause()` när den är pausad. (2) **Viloläge/stängt-skärm** (`sleep_screen.dart`) ombyggd till nya flat svartvita temat: ljus/mörk-medveten (vit/`#09090B`), ambient-glow + pulsanimation borttagna, klocka/countdown i ink, monokroma outlined badges (PAUSAD/STÄNGT), monokroma förläng-chips, **öppna-knapp = solid ink** (var grön). (3) **App-ikon**: orange kanter berodde på gammal generering med orange adaptiv-bakgrund (`#EEAE3C`) som syntes i 16%-insetens kant — bakgrunden är nu svart `#040505`. Adaptiv-foreground ombyggd från full-bleed wordmark → wordmark 70% centrerad på svart tile så guld-pricken har marginal från kanten; ikoner regenererade (Android + iOS). (4) **Logga** (`levera_logo.png`) regenererad skarp 1024px från källan (var 512px med extra padding → suddig i parnings-skärmen). APK byggd lokalt. |
| 2026-06-02 | **svartvit navbar + meny-toggle + historik idag/igår + småfixar**: (1) **Navbaren** helt svartvit — vald flik är nu en solid `ink`-pill med kontrasterande text/ikon (var guld-highlight). (2) **Meny-sidan** (`menu_screen.dart`) avfärgad: laddnings-/refresh-spinner ink, Artiklar/Tillbehör-väljaren = monokrom ink-segment (matchar historik), artikel-/tillbehörsraderna har en **enkel monokrom toggle-switch** (`_MonoSwitch`, på = ink-spår + knopp, av = muted) istället för guld stripe + PÅ/AV-pill. (3) **Historik** har nu en **Idag/Igår-väljare** (`_DayToggle`, monokrom segment) som växlar mellan dagens och gårdagens ordrar; kr-kortet ersatt med en liten `_RevenueRow` (label + antal + 20px summa). Order-detalj ordernummer i historik var redan nedkrympt (26px). (4) **Dashboard** "Inga väntande ordrar" förenklad till en flat bordered rad (muted check + text); subtext "Ni får ett ljudlarm när nästa kommer in." + grön chip borttagna. (5) **Avhämtning**: `_stepsFor('PICKUP')` slutar nu på **Klar** (Inkommen → Tillagas → Klar) — inget "Hämtad"-steg. APK byggd lokalt. |
| 2026-06-02 | **flat tema forts. — detalj/print/alert/accept**: Order-detalj svartvit (status-strip + total + kund + artiklar + summa monokroma; bara typ-pill + status-ord + förbeställning (lila) + allergener (röd) bär färg). Print-inställningar avfärgad (ikon-chips + aktiv-skrivare-kort neutrala, inte längre lila/blå/guld/teal/grön). Ny-order-alarm preorder-medveten (lila). Accept-resultat ombyggt från helgrön/amber skärm → neutral bakgrund med färgad hastighets-ikon/badge. Meny lämnad (guld-highlights = önskat). Kvar: sleep-skärm + printer-help. |
| 2026-06-02 | **flat tema i hela appen + fix historik-filter**: (1) Färgsystem: `AppTheme.preorder` (#8B5CF6 lila) + `OrderUi.colorFor(order)` = lila för förbeställning (scheduledFor), annars blå (leverans) / guld (avhämtning). Applicerat på order-kort (square/hero/queue), `OrderListTile` + advance-knapp, order-take (typ-etikett + ACCEPTERA + tid-strip), order-detalj (hero/total/status), historik-rader. (2) Flat: navbaren avblurrad (solid bar, guld-highlight på vald flik), historik-statkorten av-skuggade. Innehåll svartvitt, färg bara på typ/status/knappar. (3) **Historik-bugg fixad**: `_isTestOrder` filtrerade bort riktiga ordrar vars notering innehöll "testorder"/"test-order" (+ rabattkod test, namn "test jari") → de försvann efter "på väg". Nu döljs ENDAST lokalt simulerade `mock_`-ordrar; riktiga ordrar sparas i historiken (idag/igår). |
...
| 2026-06-03 | **launch-fixar (Delivera Business)**: (1) **Dubbel-ljud-bugg 100% fixad** — `AudioHelper` har nu tre STRIKT separerade spelare (`_loopPlayer` = ny-order-loop, `_oneShotPlayer` = alltid release-läge, `_testPlayer` = test 5s). Tidigare delade `playAudio`/`playTest` spelare → ett test satte loop-läge → nästa one-shot fastnade i loop som inte gick att stoppa. Dessutom togs det extra `playAudio(_selectedAlarm)` på `order:new` bort (spelades samtidigt som loopen → dubbelt). Larmet spelas nu EN gång och loopar tills ordern godkänns. (2) **Autoprint** skrivs nu ut AUTOMATISKT exakt vid godkännandet (PENDING→ACCEPTED/PREPARING, en gång) via `forceToPrinter:true` → alltid till konfigurerad skrivare oavsett auto-print-toggeln, ingen PDF-popup, tyst om ingen skrivare. (3) **Connection-lost**: röd helskärm (`_DisconnectOverlay`) + lugn signal (`disconnect.wav`, ej ny-order-ljudet) var 10:e sek — ENDAST under öppettider; tryck tystar, re-alert efter 3 min om fortf. offline. Utanför öppettider bara röd skärm, tyst. Logik i `OrderProvider` (`_handleConnectivity`/`acknowledgeDisconnect`), wire:at i `onConnect/onDisconnect/onConnectError`. (4) **Navbar**: fade+upp-glid-animation vid flikbyte (IndexedStack behåller state, `_tabAnim`-controller). (5) **Parningssidan** flat ink-knapp (`_FlatPrimaryButton`) istf guld `EmberButton`. (6) **Namn → "Delivera Business"** (Android label, iOS CFBundleDisplayName, MaterialApp-title, rail-monogram L→D); **version 1.34.7+47 → 26.06.03+48** (release). Interna `com.matgo.restaurant`/`matgo_token`/channelId MEDVETET kvar. (7) **Logga bytt** — "Delivera business logo.png" (1254px, svart bg #0C0B0C) → `app_icon.png` + `delivera_logo.png` (full-bleed 1024) + `app_icon_foreground.png` (wordmark 70% centrerad på #0C0B0C-tile så adaptiv-masken ej klipper). `adaptive_icon_background` #040505→#0C0B0C. `flutter_launcher_icons` regenererat (Android+iOS). Parningssidan använder nu `delivera_logo.png`. APK byggd (release, 34.5MB). |
...
| 2026-06-03 | **connection-lost förfinad + mjukt ljud + Delivera-namn**: (1) `_DisconnectOverlay` (main.dart) ombyggd → enkel, ren, lugn statisk röd (#B22A22), ingen aggressiv blink (bara mjuk andning på ikon-cirkeln), inget guld. (2) Nytt mjukt larm `assets/audio/soft_alert.wav` (varm nedåtgående ding-dong, mjuk attack/decay) ersätter `disconnect.wav`; haptik → `selectionClick`. (3) Kvarvarande "Levera Business" → "Delivera Business" i inställningar (version-rad), foreground-notis, test-kvitto, app-titel-konstant, dashboard-fallback, printer-help-support. APK byggd (release, 34.6MB). |
