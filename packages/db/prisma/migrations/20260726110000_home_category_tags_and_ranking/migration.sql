-- Spegel av packages/api/prisma/migrations/20260726110000_home_category_tags_and_ranking.
...
CREATE TABLE "RestaurantTagAssignment" (
  ...
  CONSTRAINT "RestaurantTagAssignment_tagId_fkey"
    FOREIGN KEY ("tagId") REFERENCES "RestaurantTag"("id") ON DELETE CASCADE ON UPDATE CASCADE
);
...
WITH legacy_names AS (
  ...
  WHERE jsonb_typeof(pg_temp.viaeats_safe_jsonb(r."tags")) = 'array' AND trim(tag_name) <> ''
  ORDER BY lower(trim(tag_name)), trim(tag_name)
)
INSERT INTO "RestaurantTag" ("id", "name", "slug", "color", "isActive", "sortOrder", "createdAt", "updatedAt")
SELECT
  'legacy-tag-' || substr(md5(normalized), 1, 20),
  name,
  COALESCE(NULLIF(trim(BOTH '-' FROM regexp_replace(normalized, '[^a-z0-9]+', '-', 'g')), ''), 'tag')
...
SELECT r."id", t."id", tag_row.ordinality::integer - 1, CURRENT_TIMESTAMP
FROM "Restaurant" r
...
JOIN "RestaurantTag" t ON lower(t."name") = lower(trim(tag_row.tag_name))
WHERE jsonb_typeof(pg_temp.viaeats_safe_jsonb(r."tags")) = 'array' AND trim(tag_row.tag_name) <> ''
