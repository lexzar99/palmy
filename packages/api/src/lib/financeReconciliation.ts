  ...
  for (const order of input.orders) {
    ...
    if (!terminal && settled && total - Math.max(0, refund) > 0) {
      deviations.push({
        ...
        orderNumber: order.orderNumber,
        paymentId: order.molliePaymentId,
        title: `Betald order utanför ekonomin`,
        detail: `Order ${order.orderNumber} har en verklig betalning men orderstatus ${normalized(order.status) || 'OKÄND'}. Kontrollera leverans eller återbetalning.`,
  ...
  if (input.feeStatus === 'unavailable' && ordersByPaymentId.size > 0) {
    ...
    for (const [restaurantId, paymentOrders] of missingByRestaurant) {
      ...
      deviations.push({
        ...
        orderNumber: null,
        paymentId: null,
        title: `Mollie-avgift saknas för ${uniquePaymentIds.length} betalningar`,
        detail: `Betalningarna finns i perioden men någon avgift har ännu inte bokförts eller kunde inte matchas. Provision efter avgift lämnas tom tills underlaget är komplett.`,
        amountOre: null,
        affectedOrderCount: paymentOrders.length,
        confirmedLoss: false,
      });
